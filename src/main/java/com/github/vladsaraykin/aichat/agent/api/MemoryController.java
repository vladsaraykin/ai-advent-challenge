...
public class MemoryController {
    ...
    @GetMapping("/memory") public LongTermMemory memory(@PathVariable String agentId) { return service.memory(agentId); }
    @PutMapping("/memory/{id}") public LongTermMemory put(@PathVariable String agentId, @PathVariable UUID id,
            @Valid @RequestBody EntryEdit request) {
        return service.putMemory(agentId, request.version(), id, request.scope(), request.projectKey(), request.key(), request.value());
    }
    @DeleteMapping("/memory/{id}") public LongTermMemory delete(@PathVariable String agentId, @PathVariable UUID id,
            @Valid @RequestBody Version request) { return service.deleteMemory(agentId, request.version(), id); }
    @PutMapping("/chats/{chatId}/task") public Chat task(@PathVariable String agentId, @PathVariable UUID chatId,
            @Valid @RequestBody TaskEdit request) {
        return service.editTask(agentId, chatId, request.version(), request.projectKey(), request.task().data());
    }
    @PostMapping("/chats/{chatId}/task/advance") public Chat advance(@PathVariable String agentId, @PathVariable UUID chatId,
            @Valid @RequestBody Version request) { return service.advanceTask(agentId, chatId, request.version()); }
    @PostMapping("/chats/{chatId}/proposals/{id}/accept") public LongTermMemory accept(@PathVariable String agentId,
            @PathVariable UUID chatId, @PathVariable UUID id, @Valid @RequestBody Confirmation request) {
        return service.acceptProposal(agentId, chatId, request.version(), request.taskVersion(), id);
    }
    @PostMapping("/chats/{chatId}/proposals/{id}/reject") public Chat reject(@PathVariable String agentId,
            @PathVariable UUID chatId, @PathVariable UUID id, @Valid @RequestBody Version request) {
        return service.rejectProposal(agentId, chatId, request.version(), id);
    }
}
